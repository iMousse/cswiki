...
```mermaid
sequenceDiagram
participant ds as 前端控制器
participant hm as 处理映射器
participant hd as 处理适配器
participant h as 处理器
participant vr as 视图解析器 
participant v as 视图

ds ->> +hm:  请求HandlerMapper获取Handler
hm ->> hm:   根据配置或注解找到最终要执行的Handler
hm -->> -ds: 返回Handler
ds ->> +hd:  请求HandlerAdapter执行Handler
hd ->> +h:   执行Handler
h ->> h:	   根据Handler规则执行不同类型的Handler	 
h -->> -hd:  Handler执行完毕返回一个ModelAndView
hd -->> -ds: 返回ModelAndView
ds ->> +vr:  请求进行视图解析
vr ->> vr:   将逻辑视图解析成真正的视图国际化处理
vr -->> -ds: 返回View 
ds ->> v:    渲染视图将model数据转换为response响应
```
...
1. 服务器提供了 DispatcherServlet，它使用的是标准 Servlet 技术
   ...
   * 初始化：DispatcherServlet 初始化时会优先到容器里寻找各种组件，作为它的成员变量
     ...
     * HandlerAdapter，调用控制器方法或者Handler方法。初始化时准备参数解析器、返回值处理器、消息转换器
...
![](assets/image-20211021160145072.png)

接下来勾选 Spring Web 支持

![](assets/image-20211021162416525.png)
